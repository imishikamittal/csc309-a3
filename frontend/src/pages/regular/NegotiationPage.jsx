    ...
    return (
      <div className="max-w-xl mx-auto">
        <div className="card text-center py-12">
          ...
          {negotiation?.status === 'success' && (
            <p className="text-gray-500 dark:text-gray-400 mt-2">The job has been filled. Check your commitments.</p>
          )}
          <Link to="/interests" className="btn-primary mt-6 inline-flex">Go to My Interests</Link>
        </div>
      </div>
    );
  }

  const myDecision = negotiation.decisions?.candidate;
  const otherDecision = negotiation.decisions?.business;
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card mb-4">
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider font-medium">Business</p>
            <p className="font-semibold text-gray-900 dark:text-white mt-1">{negotiation.job?.business?.business_name}</p>
          </div>
          <div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex gap-4">
          ...
          <div className="flex-1 text-center">
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider font-medium mb-1">Business decision</p>
      ...
      <div className="card">
        ...
        <div className="min-h-[200px] max-h-[300px] overflow-y-auto space-y-2 mb-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
          {messages.length === 0 ? (
            ...
            messages.map((msg, i) => {
              const isMe = msg.sender?.role === 'regular';
              return (
